...
void rb::DebugDrawNode::DrawPolygon(const b2Vec2* vertices, int32 vertexCount, const b2Color& color)
{
	auto vec = Utilities::convertToCCVecArr(vertices, vertexCount, _worldNode->PTM_RATIO);
	_drawNode->drawPoly(vec, vertexCount, true, rb::Utilities::convertToCCColor(color));
}

void rb::DebugDrawNode::DrawSolidPolygon(const b2Vec2* vertices, int32 vertexCount, const b2Color& color)
{
	auto vec = Utilities::convertToCCVecArr(vertices, vertexCount, _worldNode->PTM_RATIO);
	Color4F fCol = Utilities::convertToCCColor(color);
	Color4F bCol(fCol);
	fCol.a = 0.5f;
	_drawNode->drawPolygon(vec, vertexCount, fCol, 0.5, bCol);
}

void rb::DebugDrawNode::DrawCircle(const b2Vec2& center, float radius, const b2Color& color)
{
	Vec2 centr = _worldNode->convertToCCVector(center);
	float rad = radius * _worldNode->PTM_RATIO;
	_drawNode->drawCircle(centr, rad, 0, 16, true, Utilities::convertToCCColor(color));
}

void rb::DebugDrawNode::DrawSolidCircle(const b2Vec2& center, float radius, const b2Vec2& axis, const b2Color& color)
{
	Vec2 centr = _worldNode->convertToCCVector(center);
	float rad = radius * _worldNode->PTM_RATIO;
...
void rb::DebugDrawNode::DrawSegment(const b2Vec2& p1, const b2Vec2& p2, const b2Color& color)
{
