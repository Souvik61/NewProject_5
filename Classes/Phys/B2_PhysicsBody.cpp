...
{
    CCASSERT(_owner != nullptr, "No owner");
...
inline void rb::B2PhysicsBody::updateTransform()
{
    //updatePos();
    _owner->setPosition(_b2World->convertToCCVector(_body->GetPosition()));
    _owner->setRotation(CC_RADIANS_TO_DEGREES(-_body->GetAngle()));
}
...
{
    _body->SetTransform(b2Vec2(x / _b2World->PTM_RATIO, y / _b2World->PTM_RATIO), _body->GetAngle());
    updatePos();
}
...
{
    _body->ApplyForce(Utilities::convertToB2Vec2(1, force), Utilities::convertToB2Vec2(_b2World->PTM_RATIO, wP), wake);
}
