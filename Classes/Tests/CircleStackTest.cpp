...
cocos2d::Scene* CircleStackTest::createScene()
{
	return CircleStackTest::create();
}

bool CircleStackTest::init()
{
	if (!Scene::init())
	{
		return false;
	...
	_mouseListener->onMouseMove = CC_CALLBACK_1(CircleStackTest::onMouseMove, this);
	_mouseListener->onMouseUp = CC_CALLBACK_1(CircleStackTest::onMouseUp, this);
	_mouseListener->onMouseDown = CC_CALLBACK_1(CircleStackTest::onMouseDown, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(_mouseListener, this);

	
	int e_rowCount = 10;

	Vec2 x(visibleSize.width / 2 + origin.x, 50);
...
void CircleStackTest::update(float delta)
{
	getChildByName<DrawNode*>("dD")->setPosition(m_mouseWorld - getPosition());
}

Node* CircleStackTest::addCirAtPosition(cocos2d::Vec2 pos)
{
	b2CircleShape shape = ShapeHelper::createCircleShape(wN->PTM_RATIO, 25);
...
void CircleStackTest::onMouseUp(cocos2d::EventMouse* event)
{
...
void CircleStackTest::onMouseMove(cocos2d::EventMouse* event)
{
...
void CircleStackTest::onMouseDown(cocos2d::EventMouse* event)
{
