...
cocos2d::Scene* PyramidTest::createScene()
{
	return PyramidTest::create();
}

bool PyramidTest::init()
{
	if (!Scene::init())
	{
		return false;
	...
	_mouseListener->onMouseMove = CC_CALLBACK_1(PyramidTest::onMouseMove, this);
	_mouseListener->onMouseUp = CC_CALLBACK_1(PyramidTest::onMouseUp, this);
	_mouseListener->onMouseDown = CC_CALLBACK_1(PyramidTest::onMouseDown, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(_mouseListener, this);

	int e_count = 20;

	Vec2 x((visibleSize.width / 2 + origin.x) - 300, 12.5);
	Vec2 y;
	Vec2 deltaX(15, 35);
	Vec2 deltaY(30, 0.0f);
...
void PyramidTest::update(float delta)
{
	getChildByName<DrawNode*>("dD")->setPosition(m_mouseWorld - getPosition());
}

Node* PyramidTest::addBoxAtPosition(cocos2d::Vec2 pos)
{
	auto comp1 = SuperPhysHelper::createBox(wN, Size(25, 25), PhysicsMaterial(1, 0, 0.2f), Vec2(), pos);
...
void PyramidTest::onMouseUp(cocos2d::EventMouse* event)
{
...
void PyramidTest::onMouseMove(cocos2d::EventMouse* event)
{
...
void PyramidTest::onMouseDown(cocos2d::EventMouse* event)
{
